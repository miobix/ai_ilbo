...
export default async function handler(req, res) {
  ...
  try {
    ...
    const baseUrl = process.env.NEXT_PUBLIC_AI_SERVER_BASE_URL || 'https://skilled-amazingly-whippet.ngrok-free.app';
    const token = (process.env.NEXT_PUBLIC_NAMGYU_FAST_API_KEY || '').replace(/^Bearer\s+/i, '').trim();
    if (!token) {
